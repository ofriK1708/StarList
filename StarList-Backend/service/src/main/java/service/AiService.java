...
public class AiService {
    ...
    /** Caps prompt growth now that every frequency — not just DAILY — can raise a notice. */
    private static final int MAX_HABIT_NOTICES = 8;
    ...
    private static List<ChatCompletionTool> buildTools() {
        return List.of(
                ...
                tool("get_habits", "Get all active habits with streaks plus current-period state: "
                        + "due date, whether it is already completed this period, and how many days overdue. "
                        + "Trust those fields directly — never infer completion from lastCompletedDate.", noParams()),
                ...
                tool("create_habit", "Create a new habit for the user",
                        params(Map.of(
                                ...
                                "description", Map.of("type", "string"),
                                "frequency", Map.of("type", "string", "description", "DAILY, WEEKLY, MULTI_DAY, or CUSTOM"),
    ...
    public AiChatResponse getDailyBriefing(Long userId) {
        ...
        return chat(userId, AiChatRequest.builder()
                .message("Give me a concise daily briefing: list tasks due today or tomorrow, "
                        + "habits that are due today or already overdue, and any habit period ending soon. "
                        + "Include weekly, multi-day and custom-interval habits, not just daily ones, and use "
                        + "each habit's doneThisPeriod flag — never list something already completed. "
                        + "Finish with one short motivational note. "
    ...
    private String buildSystemPrompt(UserEntity user,
        ...
        if (habitList.isEmpty()) {
            ...
            habitList.forEach(h -> sb.append(formatHabitLine(h)));
        }
    ...
    private List<String> buildCoachNotices(List<service.dto.TaskResponse> tasks,
        ...
        habits.stream()
                // Every frequency is eligible now. The old DAILY-only filter is exactly why WEEKLY,
                // CUSTOM and MULTI_DAY habits never showed up in the briefing.
                .sorted(Comparator
                        .comparingInt((HabitResponse h) -> h.periodStatus() == null ? 0 : -h.periodStatus().daysLate())
                        .thenComparingInt(h -> h.periodStatus() == null
                                ? Integer.MAX_VALUE : h.periodStatus().daysUntilDue()))
                .map(h -> HabitCoachNotices.forHabit(h, today))
                .flatMap(Optional::stream)
                .limit(MAX_HABIT_NOTICES)
                .forEach(notices::add);
    ...
    /**
     * Renders one habit for the system prompt. Period state is spelled out explicitly so the model
     * never has to work out whether {@code lastDone} falls inside the current period — getting that
     * wrong is what made it tell users to complete habits they had already finished.
     */
    private String formatHabitLine(HabitResponse h) {
        ...
        var status = h.periodStatus();
        if (status == null) {
            return line.append("\n").toString();
        }
        if (!status.scheduledToday()) {
            line.append(" | NOT scheduled today");
        } else {
            line.append(String.format(" | period:[%s..%s] | due:%s | doneThisPeriod:%s",
                    status.periodStart(), status.periodEnd(), status.dueDate(),
                    status.completedThisPeriod() ? "YES" : "NO"));
            if (status.daysLate() > 0) {
                line.append(" | OVERDUE by ").append(status.daysLate()).append("d");
            }
        }
        return line.append("\n").toString();
    }
